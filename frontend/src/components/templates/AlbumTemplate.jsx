...
export default function AlbumTemplate({ data, item, index }) {
  const {
    setPlayingTrackData,
    playingTrack,
    chooseTrack,
  ...
  return (
    <li className="relative flex h-fit cursor-pointer flex-row items-center justify-between py-2 transition-all hover:bg-stone-200 active:bg-stone-200 dark:hover:bg-neutral-900 dark:active:bg-neutral-900">
      <div className="flex h-full w-full flex-row items-center px-8">
        <Link
          to={`/album/${item.track ? item.track.album.id : item.id}`}
          className="relative"
        >
          <img
            src={
              item.track ? item.track.album.images[1].url : data.images[1].url
            }
            alt="album-cover"
        ...
        >
          ...
          <h4 className="line-clamp-1 text-xs">{`${
            item.track ? item.track.album.name : item.name
          }`}</h4>
          <h6 className="line-clamp-1 text-xs font-light italic">{`${
            item.track ? item.track.album.artists[0].name : item.artists[0].name
          }`}</h6>
        </button>
